...
int main() {
    ...
    *(((char*) ubicacion_mapeo)+DESPL_ARCHIVO) = 'X';
    ...
    // Las modificaciones puede que no se escriban en el archivo hasta que se ejecute la función munmap. Para conseguir una actualización inmediata del archivo debes utilizar msync.
    if (msync(ubicacion_mapeo, info.st_size, MS_SYNC)) {
